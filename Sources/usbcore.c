        ...
        case HID_HID_DESCRIPTOR_TYPE:
          if (SetupPacket.wIndex.WB.L >= USB_HID_IF_NUM) 
            ...
            return (__FALSE);  
					...
					if(SetupPacket.wIndex.WB.L==1)
					{
						EP0Data.pData = (U8 *)USB_ConfigDescriptor + 0x002b;
						len = HID_DESC_SIZE;						
					}
          break;
        case HID_REPORT_DESCRIPTOR_TYPE:
          if (SetupPacket.wIndex.WB.L >= USB_HID_IF_NUM)
            ...
            return (__FALSE);  
					...
					if((InitHIDev==USE_BOTH) || (InitHIDev==USE_KEYBOARD))
					{
						if(SetupPacket.wIndex.WB.L==0)
						{
						...
						if(SetupPacket.wIndex.WB.L==1)
						{
									EP0Data.pData = (U8 *)HID_ReportDescriptorM;
									len = HID_ReportDescSizeM;					
						}
					}
					if(InitHIDev==USE_MOUSE)
					{
									EP0Data.pData = (U8 *)HID_ReportDescriptorM;
									len = HID_ReportDescSizeM;											
					}
              ...
              //if (SetupPacket.wIndex.WB.L == USB_HID_IF_NUM) 
                ...
                switch (SetupPacket.bRequest) 
                  ...
                  case HID_REQUEST_GET_REPORT:
                    if (HID_GetReport(SetupPacket.wIndex.WB.L)) 
										{
              ...
              if (SetupPacket.wIndex.WB.L == USB_MSC_IF_NUM) /* IF number correct? */
                ...
                switch (SetupPacket.bRequest) 
                  ...
                  case MSC_REQUEST_RESET:
                    if ((SetupPacket.wValue.W == 0) && (SetupPacket.wLength  == 0)) /* RESET with invalid parameters -> STALL */
									  {
                      if (MSC_Reset()) 
											{
                        USB_StatusInStage();
                  ...
                  case MSC_REQUEST_GET_MAX_LUN:
                    if ((SetupPacket.wValue.W == 0) && (SetupPacket.wLength  == 1)) /* GET_MAX_LUN with invalid parameters -> STALL */      
									  { 
                      if (MSC_GetMaxLUN()) 
											{
                        EP0Data.pData = EP0Buf;
...
out_class_ok:                                                            /* request finished successfully */
    ...
    case USB_EVT_IN :
				//	if (SetupPacket.bRequest==0x09)
					//	if (SetupPacket.bmRequestType.B==0x22)
							//MDR_PORTC->RXTX	|=0x01;			
//			MDR_PORTD->RXTX ^= (1 << 14);
      if (SetupPacket.bmRequestType.BM.Dir == REQUEST_DEVICE_TO_HOST) 
			{
